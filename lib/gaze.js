  ...
  for (var i = 0; i < files.length; i++) {
    ...
    for (var j = 0; j < readdir.length; j++) {
      ...
      try {
        ...
        if (err.code === 'ENOENT') {
          // This is a non-existent file we are trying to stat.
          // Either it has been deleted prior to this stat or is a broken link
          this._handleWarning(err);
      } else {
...
// If an warning, handle it here
Gaze.prototype._handleWarning = function(err) {
  if (err.code === 'EMFILE') {
    return this.emit('warning', new Error('EMFILE: Too many opened files.'));
  }
